...
class queries:
    def __init__(self):
        self.euclidean_object = calc_distance()
        pass

    def query_ret_k_close(self, sorted_tree, k):
        '''
    ...
    def QUERY(self, q, uvecs, trees, sorted_trees, q_projs, D, k0, k1, k):
        ...
        for l in range(L):  # L * m * (log(n) + log(m))
            for j in range(m):
                ...
                p_proj, pt = self.query_ret_k_close(sorted_trees[j, l], 0)
            ...
            for l in range(L):
                if len(Sls[l]) < k0:
                    ...
                    if len(cp_Pl) == 0:
                        ...
                        p_proj, pt = self.query_ret_k_close(
