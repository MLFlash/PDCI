...
def extract_data(datafile, out_path):
    if datafile.endswith(".zip"):
        ...
        raise NotImplementedError(
            ".{} is not currently supportted".format(datafile.split(".")[-1]))


def load_housing_data(local_data_dir):
    data_file = os.path.join(local_data_dir, 'housing.csv')
...
if __name__ == "__main__":
    ...
    local_compressed_datafile = os.path.join(
        local_data_dir, 'california-housing-prices.zip')
    local_cached_datafile = os.path.join(local_data_dir, 'housing.csv')

    housing = load_housing_data(local_data_dir)
    housing.head(3)
    type(housing)
